...
export type WeatherRequest = {
  id?: number;
  user_id?: string | null;     // add this if you’ll use auth
...
export async function createWeatherRequest(entry: Omit<WeatherRequest, "id">) {
  const { data, error } = await supabase.from("weather_requests").insert([entry]).select().single();
  if (error) {
    console.log("[Create] error");
    throw error;
  } 
  console.log("Create inserted: ", data)
  return data;
}

export async function listWeatherRequests() {
  const { data, error } = await supabase.from("weather_requests").select("*").order("created_at", { ascending: false });
